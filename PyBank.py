...
with open(csvpath, newline='') as csvfile:
    ...
    for row in csvreader:
        ...
        # For all other lines, sum the monthly profits, calculate the profit change
        # month to month.
        else:
            monthly_profit = int(profit)
            total_profit = total_profit + monthly_profit
            ...
            # Look for the greatest profit increase. If you find one
            # greater than the current, replace the "greatest" value
            if profit_change > greatest_change:
                greatest_change = profit_change
                greatest_date = row[0]
            # Look for the greatest profit decrease. If you find one
            # greater than the current, replace the "greatest" value
            elif profit_change < least_change:
                least_change = profit_change   
                least_date = row[0] 
            previous_profit = monthly_profit
    # This calculates the average change in profits
    last_profit = int(profit)
    average_change = (last_profit - first_profit)/(month_count - 1)
    #Print out the information we calculated
